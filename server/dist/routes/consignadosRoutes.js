...
router.post("/consignados", async (req, res) => {
    ...
    try {
        ...
        const { nome, contato, telefone, email, endereco, cidade, estado, cnpj, comissao, ultima_entrega, status = 'ativo', produtos = [] // Array de produtos para consignar
         } = req.body;
        ...
        if (produtos && produtos.length > 0) {
            for (const produto of produtos) {
                ...
                const productCheck = await client.query('SELECT id, name, status, quantity FROM moari.products WHERE id = $1', [product_id]);
                ...
                const existingCheck = await client.query('SELECT * FROM moari.product_consignados WHERE product_id = $1 AND status = $2', [product_id, 'ativo']);
    ...
    `;
        ...
        `;
                ...
                await client.query('UPDATE moari.products SET status = $1 WHERE id = $2', ['consigned', product_id]);
                // Reduzir a quantidade do produto (se necessário)
                await client.query('UPDATE moari.products SET quantity = quantity - $1 WHERE id = $2', [quantidade, product_id]);
            ...
            await client.query('UPDATE moari.consignados SET ultima_entrega = CURRENT_DATE WHERE id = $1', [consignado.id]);
        }
...
router.get("/products/:productId/consignacao-info", async (req, res) => {
    const client = await pool.connect();
    try {
...
router.get("/consignados/evolucao", async (req, res) => {
    const client = await pool.connect();
    try {
