...
router.get("/diagnose-suppliers-table", async (req, res) => {
    ...
    catch (error) { // Explicitamente tipado como 'any' para acessar .message
...
router.get("/products", async (req, res) => {
    ...
    try {
        ...
        const { page = 1, limit = 10, search, category, quality, orderBy = "created_at", orderDirection = "desc" } = req.query;
...
router.post("/products", upload.array("images", 5), async (req, res) => {
    ...
    try {
        ...
        const { code, name, category, format, quality, material_type, usage_mode, size, origin, warranty, base_price, profit_margin, description, materials, } = req.body;
        const insertQuery = `
...
router.put("/suppliers/:id", (async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { nome, contato, telefone, email, cidade, estado, endereco, ultima_compra, } = req.body;
...
router.get("/suppliers", async (req, res) => {
    const client = await pool.connect();
    try {
        const { page = 1, limit = 10, search, cidade, estado, orderBy = "nome", orderDirection = "asc", } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        let queryParams = [];
        let conditions = [];
        let queryText = `
...
router.post("/suppliers", async (req, res) => {
    const client = await pool.connect();
    try {
        const { nome, contato, telefone, email, cidade, estado, endereco, ultimacompra, } = req.body;
...
router.get("/next-Supplier-id", async (req, res) => {
    const client = await pool.connect();
    try {
...
//# sourceMappingURL=fornecedoresRoutes.js.map