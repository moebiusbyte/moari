...
const CadastroConsignados = ({ isOpen, onClose, onSave, }) => {
    ...
    useEffect(() => {
        const fetchProdutos = async () => {
            ...
            try {
                ...
                const produtosDisponiveis = response.data.products.filter((p) => p.status === 'active' && (p.quantity || 0) > 0);
    ...
    return (_jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40", children: _jsxs("div", { className: "bg-white rounded-lg shadow-lg p-8 w-full max-w-2xl", children: [_jsxs("div", { className: "flex justify-between items-center mb-4", children: [_jsx("h2", { className: "text-xl font-bold", children: "Cadastro de Consignados" }), _jsx("button", { onClick: onClose, className: "text-gray-500 hover:text-gray-700", children: _jsx(X, { size: 24 }) })] }), loading && _jsx("p", { children: "Carregando..." }), error && _jsx(Alert, { children: _jsx(AlertDescription, { children: error }) }), _jsxs("form", { onSubmit: e => {
                        ...
                        onSave(consignado, produtosSelecionados);
                                                    ...
                                                    return (_jsxs("div", { className: `flex items-center gap-2 mb-2 ${quantidadeDisponivel === 0 ? 'opacity-50' : ''}`, children: [_jsx("input", { type: "checkbox", className: "w-10", checked: produtoSelecionado !== undefined, onChange: () => handleProdutoSelect(produto.id), disabled: quantidadeDisponivel === 0, title: quantidadeDisponivel === 0 ? "Produto sem estoque" : "" }), _jsx("span", { className: "flex-1", children: produto.name || produto.nome || "Sem nome" }), _jsx("span", { className: `w-20 text-center text-sm ${quantidadeDisponivel === 0 ? 'text-red-600' : 'text-gray-600'}`, children: quantidadeDisponivel }), produtoSelecionado ? (_jsx("input", { type: "number", min: 1, max: quantidadeDisponivel, className: "w-24 border rounded px-2 py-1", value: produtoSelecionado.quantidade, onChange: e => handleProdutoChange(produto.id, "quantidade", Number(e.target.value)), placeholder: "Qtd", title: `Máximo: ${quantidadeDisponivel}` })) : (_jsx("input", { type: "number", className: "w-24 border rounded px-2 py-1 bg-gray-100", disabled: true, title: "Selecione o produto primeiro" }))] }, produto.id));
                                                }) })] })) })] }), _jsx("button", { type: "submit", className: "bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700", children: "Salvar" })] })] }) }));
};
